  ...
  <template v-if="!pageLoading">
    <div class="grid grid-cols-4 md:grid-cols-1 gap-2">
      ...
      <InfoCard :loading="!!!bookStatistics" class="col-span-1" label="Sold this Month">
        ...
        <span
          v-else-if="bookStatistics['sold_last_month']['change_rate'] > 0"
          class="leading-none flex items-center text-xs px-2 py-1 rounded-lg bg-red-100 text-red-700 font-medium"
        >
        ...
        <span
          v-else
          class="leading-none flex items-center text-xs px-2 py-1 rounded-lg bg-green-100 text-green-700 font-medium"
        >
      ...
      <InfoCard :loading="!!!stores" class="col-span-1" label="Balance in store">
        <template v-if="!!stores">
          {{
            stores.filter((store) => store.store_primary)[0]
              ? stores.filter((store) => store.store_primary)[0].balance
              : 0
          }}
        </template>
      </InfoCard>
    ...
    <div class="w-full flex sm:flex-col gap-6">
      <div class="sm:w-full w-1/2 flex flex-col gap-4">
        <ItemCard v-if="!loading" class="" :title="book.title" :heading="true">
          <template #action>
            <button
              @click="goToUpdate"
          ...
          <template #rows>
            ...
            <ItemDescription label="supplier">
              ...
              <button v-else class="text-brand-primary text-xs cursor-pointer">
                Register a Supplier
              </button>
            </ItemDescription>
            <ItemDescription label="book covers">
              <template v-if="!!book['front_cover'] || !!book['back_cover']">
                <button
                  @click="setOpenUploadCoversForm(true)"
                  class="text-brand-primary text-xs cursor-pointer"
                >
                  View Book Covers
                </button>
              </template>
            ...
            <ItemDescription
              :description="new Date(book['created_at'] * 1000).toDateString()"
              label="registration date"
            />
            <ItemDescription
              :description="new Date(book['updated_at'] * 1000).toDateString()"
              label="last updated"
            />
          </template>
        </ItemCard>
      ...
      <div
        v-if="book && book?.transactions?.length > 0"
        class="w-1/2 sm:w-full flex flex-col gap-6"
      >
        <div class="flex justify-center w-full">
          <Toggle
            :labels="['Stock records', 'Store records']"
            colors="white"
            v-model="toggleStock"
          />
        </div>
        ...
        >
          ...
          <template #rows="data">
            <Cell
              name="invoice"
              :hide="data.hide"
              class="w-[4%] text-xs font-semibold text-subtitle text-center"
              >{{ data['record']['invoice'].toLowerCase() }}
            </Cell>
            <Cell
              name="quantity"
              :main="true"
              :hide="data.hide"
              class="w-[88%] text-xs font-semibold text-subtitle"
        ...
        >
          ...
          <template #rows="data">
            <LinkCell
              ...
              :main="true"
              :hide="data.hide"
              :value="data['record']['store']"
              :to="'/stores/' + data['record']['store_id']"
            >
            ...
            <EnumCell
              ...
              :hide="data.hide"
              :colors="['green', 'stone']"
              :options="['primary', 'warehouse']"
              :value="data['record']['store_primary']"
            />
            <Cell
              name="balance"
              :hide="data.hide"
              class="w-[10%] text-xs font-semibold text-black text-center"
              >{{ data['record']['balance'] }}
            </Cell>
          </template>
        </Table>
      ...
      <div
        v-else
        class="min-h-24 flex flex-col items-center justify-center gap-2 font-medium text-subtitle py-8 h-fit w-1/2 sm:w-full"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
        ...
        <h5 class="font-normal sm:w-full w-[24rem] text-center">
          There has not been any transactions made (purchase or sale) for this item. Stock and store
          records will appear here as transactions are preformed.
        </h5>
      </div>
    </div>
  </template>
...
async function fetchBook() {
  await store
    .dispatch('getBook', useRoute().params.id)
    .then(() => {
...
function goToUpdate() {
  router.push({
    name: 'UpdateBook',
    params: { id: store.state.book.book.id }
  })
}
