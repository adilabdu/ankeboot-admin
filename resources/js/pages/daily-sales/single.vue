<template>
  <!-- Toggle // "Title" -->
  <div
    v-if="chosenDate && chosenDate['is_submitted'] && !loading"
    class="relative w-full flex items-center justify-center"
  >
    <Toggle
      colors="white"
      class="!bg-white sm:z-10"
      v-model="toggleState"
      :labels="['Sales Summary', 'Sales Breakdown']"
    />
    <div
      class="absolute right-2 top-0 sm:left-0 bottom-0 flex flex-row-reverse items-center sm:justify-between sm:w-full sm:z-last gap-4"
    >
      <svg
        ...
        viewBox="0 0 24 24"
        stroke-width="1.5"
        class="cursor-pointer stroke-subtitle hover:stroke-red-500 hover:scale-125 hover:stroke-2 w-5 h-5 transition-transform duration-150"
      >
        <path
      ...
      <svg
        @click="updateDailySale"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        stroke-width="1.5"
        class="stroke-subtitle hover:stroke-black hover:scale-125 cursor-pointer w-5 h-5 transition-transform duration-150"
      >
        <path
          stroke-linecap="round"
          stroke-linejoin="round"
  ...
  <div
    v-if="!toggleState && chosenDate && chosenDate['is_submitted'] && !loading"
    class="sm:flex sm:flex-col grid grid-cols-8 w-full gap-2"
  >
    <div class="col-span-2 col-start-2 flex flex-col gap-2">
      <p class="text-center font-medium text-subtitle w-full sm:text-left sm:ml-2">
        Sales Receipts
      </p>
      ...
      <template v-for="sales_receipt in chosenDate['sales_receipts']">
        <div class="flex w-full border border-border-light rounded-xl shadow-sm h-fit bg-white/75">
          <div class="flex flex-col leading-tight items-start p-4 text-lg justify-center grow">
            <p :class="[sales_receipt['is_manual'] ? '' : '']" class="text-subtitle">
              {{ sales_receipt['receipt'] }}
            </p>
          </div>
          <div
            class="flex flex-col gap-0.5 leading-tight items-end p-4 text-lg justify-center grow"
          >
            ...
            <p
              v-if="sales_receipt['is_manual']"
              class="text-xs px-2 py-0.5 border border-brand-primary text-brand-primary rounded-full"
            >
              manual receipt(s)
            </p>
          </div>
      ...
      <template v-for="deposit in chosenDate['deposits']">
        <div class="flex w-full border border-border-light rounded-xl shadow-sm h-fit bg-white/75">
          <div class="flex flex-col leading-tight items-start p-4 text-lg justify-center grow">
            ...
            <p v-if="!!!deposit['credit']" class="text-subtitle text-xs">
              {{ new Date(deposit['deposited_on']).toDateString() }}
            </p>
            <template v-else>
              <p class="text-subtitle text-xs">
                {{ deposit['credit']['client_name'] }}
              </p>
              <p class="text-subtitle text-xs">
          ...
          <div
            class="p-4 w-fit h-full flex flex-col leading-tight items-end justify-center text-lg font-medium"
          >
            ...
            <template v-if="!!deposit['credit']">
              <p
                v-if="deposit['credit']['submitted_on']"
  ...
  <div
    v-if="toggleState && chosenDate && chosenDate['is_submitted'] && !loading"
    class="w-full flex md:flex-col sm:gap-4 items-start gap-2"
  >
    <ItemCard class="" :title="'Daily Sales Report'" :heading="false">
      <template #rows>
        ...
        <ItemDescription
          ratio="5:7"
          :description="new Date(chosenDate['date_of']).toDateString()"
          label="Daily Sales date"
        />
        <ItemDescription ratio="5:7" label="Submission date">
          ...
          <label
            v-else
            class="px-2 py-0.5 rounded-full bg-brand-secondary text-brand-primary text-xs"
          >
            {{ 'not submitted' }}
          </label>
        </ItemDescription>
        ...
        <ItemDescription ratio="5:7" label="Sold">
          <label class="font-medium" v-if="chosenDate['transactions'].length > 0">
            ...
            <span class="text-xs text-subtitle">
              ({{
                chosenDate['transactions'].reduce((prev, next) => {
        ...
        <ItemDescription ratio="5:7" label="Deposit receipts">
          <label class="font-medium">
            ...
            {{
              formatPrice(
                parseFloat(
                  chosenDate['deposits'].reduce((old, value) => {
                    return old + value['amount']
                  }, 0)
                )
              )
            }}
        ...
        <ItemDescription ratio="5:7" label="Expenses from daily sale">
          <label class="font-medium">
            ...
            {{
              formatPrice(
                parseFloat(
                  chosenDate['expenses'].reduce((old, value) => {
                    return old + value['amount']
                  }, 0)
                )
              )
        ...
        <ItemDescription ratio="5:7" label="Sales receipts">
          <label class="font-medium">
            ...
            {{
              formatPrice(
                parseFloat(
                  chosenDate['sales_receipts'].reduce((old, value) => {
                    return old + value['amount']
                  }, 0)
                )
              )
  ...
  <Form
    :loading="submitting"
    v-if="chosenDate && !chosenDate['is_submitted'] && !loading"
    :submit="submitDailySale"
    title="Submit Daily Sale"
  >
    <template #subtitle>
      Daily sales record for
      <span class="text-brand-primary"> {{ new Date(chosenDate['date_of']).toDateString() }} </span
      >. Record the <b><i>sales receipts</i></b
      >, <b><i>deposits</i></b
      >, <b><i>credit sales</i></b
    ...
    <div class="flex sm:flex-col items-center justify-center w-full gap-2">
      <TextInput
        ...
        disabled
        label="Date of Sale"
        v-model="newDailySale.date"
        placeholder="Name of cashier on call"
      />
      <TextInput
        class="w-3/5 sm:w-full"
        required
        label="Cashier"
        v-model="newDailySale.cashier"
        placeholder="Name of cashier on call"
      />
    </div>
      ...
      <template v-for="(sale, index) in newDailySale['sales']" :key="index">
        <div class="relative flex sm:flex-col items-start justify-center w-full gap-2">
          ...
          <PriceInput
            class="w-2/5 sm:w-full"
            required
            :label="index === 0 ? 'Z-Report amount (ETB)' : 'Sale amount (ETB)'"
            v-model="sale.amount"
          ...
          <TextInput
            ...
            required
            label="Receipt No."
            v-model="sale.receipt"
            :placeholder="index === 0 ? 'Receipt of Z-Report (Z:****)' : 'Sale receipt no.'"
          />
          <button
            type="button"
            @click="removeSalesReceipt(index)"
            v-if="index !== 0"
            class="sm:absolute sm:top-0 sm:mt-6 sm:order-first self-end rounded-md min-h-[2.5rem] min-w-[2.5rem] flex items-center justify-center"
          >
            <svg
              width="15"
              height="17"
        ...
        <div class="flex flex-col w-full gap-2">
          <div class="flex sm:flex-col items-start justify-start w-full gap-2">
            <button
              ...
              v-if="index !== 0"
              class="self-end rounded-md h-10 w-10 flex items-center justify-center"
            >
              <svg
                width="15"
                height="17"
            ...
            <PriceInput
              ...
              placeholder="Total deposit amount in ETB"
            />
            <DatePicker
              label="Deposit date"
              required
              v-model="deposit.deposited_on"
              class="w-1/3 sm:w-full"
            />
            <Dropdown
              ...
              :list="['cash', 'withholding', 'cheque', 'transfer', 'credit', 'card']"
              placeholder="Choose the type of Deposit"
              label-direction="top"
              label="Deposit type"
              class="!w-1/3 sm:!w-full h-16"
            />
          </div>
          <div v-if="deposit.type === 'credit'" class="flex items-start justify-start w-full gap-2">
            <div
              v-if="index !== 0"
              class="self-end rounded-md h-10 min-w-[2.5rem] flex items-center justify-center"
            />
            <TextInput
              class="w-full"
              required
              label="Credit receipt"
              v-model="deposit.receipt"
              placeholder="Receipt of the credit attachment (FS****)"
            />
            <TextInput
              class="w-full"
              required
              label="Credited To"
              v-model="deposit['client_name']"
              placeholder="Client name the sale was credited to"
            />
          </div>
        </div>
      </template>
        ...
        <template v-for="(expense, index) in newDailySale['expenses']" :key="index">
          <div class="flex sm:flex-col items-start justify-start w-full gap-2">
            ...
            <PriceInput
              ...
              placeholder="Total expense amount in ETB"
            />
            <TextInput
              ...
              placeholder="Payment voucher receipt issued for expense (PV****)"
            />
          </div>
          <div class="flex items-start justify-start w-full gap-2">
            <div
              v-if="index !== 0"
              class="self-end rounded-md h-10 sm:hidden min-w-[2.5rem] flex items-center justify-center"
            />
            <TextAreaInput
              class="w-full"
              required
  ...
  <template v-if="toggleState && chosenDate && !loading">
    <form
      v-if="!chosenDate['transactions'].length > 0"
      @submit.prevent="insertDailyTransaction"
      title="Insert Daily Transaction"
      submit=""
      class="sm:flex sm:flex-col w-full grid grid-cols-12 gap-2"
    >
      <div class="col-span-4 py-2 flex flex-col gap-1">
        <h1 class="text-base">Insert multiple Book records from CSV</h1>
        <slot name="subtitle">
          <h2 class="text-subtitle">
            Upload an excel or comma separated value (CSV) file to record the sale transactions for
            <span class="text-brand-primary">
              {{ new Date(chosenDate['date_of']).toDateString() }} </span
            >. Make sure to include all the necessary columns and follow the template to insert the
            data successfully.
          </h2>
        </slot>
      </div>
    ...
    >
      ...
      <template #rows="data">
        <Cell
          ...
          >{{ data['record']['book_code'].toLowerCase() }}
        </Cell>
        <Cell class="w-[90%]" name="title" :main="true" :hide="data.hide"
          >{{ data['record'].title }}
        </Cell>
        ...
        <EnumCell
          ...
          :value="data['record']['book_type']"
        />
        <LinkCell
          class="w-[2%] text-xs px-6 text-brand-primary"
          name="stock_card"
          :hide="data.hide"
          :to="'books'"
          value="books"
        >
          <RouterLink
            :to="{
              name: 'Book',
              params: { id: data['record']['stock_card'] }
            }"
          >
            {{ 'Stock card' }}
          </RouterLink>
  ...
  <!-- Loading indicator -->
  <LoadingIndicator :opacity="0.75" v-if="loading" />
</template>
...
async function submitDailySale() {
  ...
  const recordedDailySale = {
    ...
    deposits: newDailySale.value.deposits
      .filter((deposit) => {
    ...
    credits: newDailySale.value.deposits
      .filter((deposit) => {
        return deposit.type === 'credit'
      })
      .map((deposit) => {
  ...
  await axios
    .post('/api/daily-sales/update', payload)
    .then(() => {
