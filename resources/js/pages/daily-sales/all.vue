<template>
  <div class="w-full h-fit flex flex-col gap-2 items-end">
    <div class="w-full grid grid-cols-4 gap-2 sm:grid-cols-1 sm:grid-rows-1 relative">
      <div
        class="top-0 left-0 h-full py-1 -translate-x-[2.5rem] absolute flex flex-col justify-between"
      >
        <button
          @click="decreaseRange"
          :disabled="range === 0"
          class="scale-95 focus:outline-none transition-shadow duration-150 disabled:hover:shadow-sm hover:shadow-md disabled:opacity-50 p-2 rounded-lg border border-border-dark bg-white shadow-sm"
        >
          <ChevronUpIcon class="scale-110 stroke-subtitle w-4 h-4" />
        </button>
        ...
        <button
          ...
          class="scale-95 focus:outline-none transition-shadow duration-150 disabled:hover:shadow-sm hover:shadow-md disabled:opacity-50 p-2 rounded-lg border border-border-dark bg-white shadow-sm"
        >
          <ChevronDownIcon class="scale-110 stroke-subtitle w-4 h-4" />
        </button>
      </div>
      ...
      <InfoCard class="group col-span-1 sm:row-span-1" label="Unsubmitted Daily Sales">
        <div v-if="!loading" class="w-full flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-base text-subtitle"> / {{ statistics['daily_sale']['count'] }} </span>
          </div>
        </div>
        <svg
          v-else
      ...
      <InfoCard class="col-span-1 sm:row-span-1">
        <template #label>
          <p class="flex gap-2 items-center">
            <span>Deposit / Report difference</span>
            <span class="text-subtitle/75">
              {{ rangeOptions[range] }}
            </span>
          </p>
        </template>
        ...
        <div class="flex justify-between items-center gap-2 w-full h-full">
          <div v-if="!loading" class="items-center gap-1 flex">
            <span class="text-base"
              >({{ statistics['aggregate'].difference.all > 0 ? '+' : '-' }})</span
            >
            <template v-if="range === 2">
              <span class="">{{
                formatPrice(Math.abs(statistics['aggregate']['difference']['last_week']))
              }}</span>
            </template>
            <template v-else-if="range === 1">
              <span class="">{{
                formatPrice(Math.abs(statistics['aggregate']['difference']['last_month']))
              }}</span>
            </template>
            <template v-else>
              <span class="">{{
                formatPrice(Math.abs(statistics['aggregate']['difference'].all))
              }}</span>
            </template>
      ...
      <InfoCard class="col-span-1 sm:row-span-1">
        <template #label>
          <p class="flex gap-2 items-center">
            <span>Unsettled Credit Sales</span>
            <span class="text-subtitle/75">
              {{ rangeOptions[range] }}
            </span>
          </p>
...
const range = ref(0)
const rangeOptions = ref(['All time', 'Last Month', 'Last Week'])
...
async function getStatistics() {
  await axios
    .get('/api/daily-sales/statistics', {
