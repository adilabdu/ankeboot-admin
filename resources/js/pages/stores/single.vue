<template>
            ...
            <div v-if="store" class="text-sm font-normal flex flex-col overflow-hidden">
                ...
                <div class="flex gap-4">
                    <span v-if="store.address" class="text-subtitle text-xs max-w-[18rem] truncate">
                        ...
                        Woreda {{ store.address.kebele }},
                        House No. {{ store.address.house_number }}
                    </span>
    ...
    >
        ...
        <template #description>
            <p>
                Complete list of books in store
            </p>
        </template>
        ...
        <template #rows="data">
            ...
            <Cell name="in_store" :hide="data.hide" class="w-[2%] text-xs font-semibold text-subtitle text-center">{{ data['record']['in_store'] }} </Cell>
...
<script setup>
    ...
    function getStoreStatistics() {
        ...
        axios.get('/api/stores/statistics', {
            params: {
                store_id: useRoute().params.id
            }
        })
