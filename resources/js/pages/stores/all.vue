<template>
  ...
  <template v-else>
    <div
      class="grid sm:grid-cols-1 sm:w-full sm:place-items-center md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 grid-cols-6 gap-4"
    >
      ...
      <button
        @click="toggleModal"
        class="w-full sm:max-w-[248px] hover:bg-white hover:backdrop-blur transition-all duration-150 h-64 rounded-xl border-dashed border-2 border-subtitle flex flex-col gap-4 justify-center items-center"
      >
    ...
    <Teleport v-if="modalState" to="#top-view">
      <Modal>
        <div
          id="formModal"
          ref="formModal"
          class="form-modal w-[45%] sm:w-full sm:rounded-t-lg sm:overflow-auto sm:max-h-[75%] sm:animate-slide-up-modal animate-scale-up"
        >
          <Form
            modal
            @cancel="toggleModal"
          ...
          >
            ...
            >
              <div class="w-full flex sm:flex-col gap-8">
                <TextInput
                  v-model="newStore.address.sub_city"
                ...
                />
                <TextInput
                  v-model="newStore.address.kebele"
                  class="sm:w-full w-1/3"
                  required
                  placeholder="Kebele"
                  label="Kebele"
                />
                <TextInput
                  v-model="newStore.address.house_number"
                  class="sm:w-full w-1/3"
              ...
              <div class="w-full sm:flex-col flex gap-8">
                <TextInput
                  v-model="newStore.address.country"
...
function submitNewStore() {
  axios
    .post('/api/stores', newStore.value)
    .then(() => {
