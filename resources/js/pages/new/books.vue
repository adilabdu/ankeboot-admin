<template>
  <Form title="Register new Book" :submit="createNewBook">
    <TextInput
      required
      label="Book title"
      placeholder="Insert new book title"
      v-model="bookData.title"
    />
    <TextInput
      label="አማርኛ አርእስት"
      placeholder="የአማርኛ አርእስቱን ያስገቡ"
      v-model="bookData.alternate_title"
    />
    ...
    <Collapsable
      ...
      @open="toggleBookMetadataRegistration"
    ...
    <Collapsable
      :open="false"
      collapse-direction="down"
      @open="toggleTransactionRegistration"
      :label="['Register first transaction', 'Register first transaction']"
    >
      <div
        class="bg-wallpaper rounded-md text-subtitle font-medium flex gap-2 items-center w-full h-48 items-center justify-center"
      >
        <p>First transaction registration form</p>
        <label
          class="text-2xs leading-none px-1.5 py-0.5 mt-0.5 font-semibold rounded-md text-red-600 bg-red-200"
          >beta</label
        >
      </div>
    </Collapsable>
    <Collapsable
      :open="false"
      collapse-direction="down"
      @open="toggleSupplierRegistration"
      :label="['Register item supplier', 'Register item supplier later']"
    >
      <div
        class="bg-wallpaper rounded-md text-subtitle font-medium flex w-full h-48 items-center justify-center"
      >
  ...
  <form
    @submit.prevent="insertMultipleBooks"
    title="Insert Multiple Book "
    submit=""
...
function createNewBook() {
  ...
  store
    .dispatch('postBook', payload.value)
    .then((response) => {
