<template>
    ...
    <Form title="Register new Transaction" :submit="createNewTransaction">
        ...
        <div class="flex items-center gap-8 w-full">
            <TextInput :disabled="transactionType.type" v-model="transactionData.quantity" required class="w-full" placeholder="Quantity of items in transaction" label="Item Quantity" />
            ...
            <template v-for="(store, index) in storesData" :key="index">
                ...
                <div class="flex sm:flex-col w-full gap-8">
                    <div class="flex sm:w-full w-1/2">
                        <button type="button" @click="removeStore(index)" v-if="index !== 0" class="mx-2 self-end rounded-md h-10 w-10 flex items-center justify-center">
                        ...
                        <Dropdown v-if="!loadingStore" class="h-16 grow" :hide-label-on-mobile=false required v-model="store.store_id" :list="stores" :resource_list="{ display: 'name', value: 'id' }" placeholder="Choose Warehouse" label-direction="top" label="Warehouse" />
                    </div>
                    <TextInput v-model="store.amount" required class="w-1/2 sm:w-full" placeholder="Quantity of items in store" label="Item Quantity" />
                </div>

            </template>

            <button type="button" @click="addStore" class="-mt-2 px-4 py-2 gap-2 rounded-full bg-brand-secondary w-fit m-auto flex items-center justify-center">
...
<script setup>
    ...
    watch([storesData, transactionType], () => {
