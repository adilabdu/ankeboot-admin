<template>
    ...
    >
        ...
        <template #description>
            <p>
                Complete list of consignment books in the inventory
            </p>
        </template>
        ...
        <template #rows="data">
            ...
            <Cell name="payable" :hide="data.hide" class="w-[2%] text-xs font-semibold text-subtitle text-center">{{ data['record']['payable'] }} </Cell>
            <Cell name="settled" :hide="data.hide" class="w-[2%] text-xs font-semibold text-subtitle text-center">{{ data['record']['settled'] }} </Cell>
...
<script setup>
    ...
    function getBooks() {
        ...
        axios.get('/api/consignments/books')
            .then(response => {
