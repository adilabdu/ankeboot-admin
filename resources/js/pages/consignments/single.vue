<template>
  <div class="grid grid-cols-4 md:grid-cols-1 gap-2">
    ...
    <InfoCard class="group col-span-1" label="Last settlement date">
      <span v-if="!!lastSettlement" class="w-full">
        <span class="w-full flex items-center justify-between">
          <label
            class="text-subtitle scale-75 origin-left"
            v-if="lastSettlement.transaction_on === 'unsettled'"
          >
            N/A
          </label>
          <template v-else>
            <div class="gap-2 flex items-center justify-center">
              <div
                class="h-12 w-12 text-[1.6rem] rounded-full bg-border-dark text-black flex items-center justify-center"
              >
              ...
              <div class="h-full flex flex-col items-start justify-between">
                <p class="text-base">
                  <template v-if="datesInEthiopian">
                    {{
                      ethiopianMonths[
                        ethiopianDate(new Date(lastSettlement.transaction_on)).getMonth()
                      ]
                    }}
                  </template>
                  <template v-else>
          ...
          <svg
            ...
            :class="[datesInEthiopian ? 'rotate-45' : '-rotate-45']"
    ...
    <InfoCard class="col-span-1" label="Payable amount in quantity">
      <span class="flex items-center gap-2" v-if="!!consignmentHistory">
        ...
        <button
          @click="toggleInfoModalCardView"
          class="hover:scale-105 hover:bg-black transition-colors transition-transform duration-150 cursor-pointer scale-90 grid place-items-center w-4 h-4 bg-subtitle rounded-full font-extrabold text-2xs text-white"
        >
  ...
  <div class="w-full flex sm:flex-col gap-4">
    <div class="sm:w-full w-1/2 flex flex-col gap-6">
      ...
      >
        <template #action>
          <button
            class="focus:outline-brand-primary sm:focus:outline-none focus:outline-offset-2 h-fit md:w-fit px-6 py-2.5 bg-brand-primary rounded-lg text-white flex justify-between sm:justify-center items-center gap-2 sm:w-full"
          >
            ...
            <span class="font-medium whitespace-nowrap"> Download Stock Record </span>
          </button>
        </template>
        ...
        <template #rows>
          ...
          <ItemDescription
            ratio="5:7"
            :description="consignmentHistory.book.category"
            label="category"
          />
          <ItemDescription
            label="balance"
            ratio="5:7"
            :description="consignmentHistory.book.balance + ' copies'"
            class="tabular-nums font-medium"
          />
          <ItemDescription ratio="5:7" :description="consignmentHistory.book.code" label="code" />
          <ItemDescription
            ratio="5:7"
            :description="consignmentHistory.book.type"
            label="purchase type"
          />
          <ItemDescription
            ratio="5:7"
            :description="new Date(consignmentHistory.book['created_at'] * 1000).toDateString()"
            label="registration date"
          />
          <ItemDescription
            ratio="5:7"
            :description="new Date(consignmentHistory.book['updated_at'] * 1000).toDateString()"
            label="last updated"
          />
        </template>
      </ItemCard>
      ...
      >
        <template #action>
          ...
          >
            ...
            <span class="font-medium whitespace-nowrap"> Update Supplier Information </span>
          </button>
        </template>
      ...
      <div
        v-else
        class="bg-wallpaper relative flex flex-col h-fit items-center justify-center gap-4 w-full border-2 border-dashed border-border-light rounded-md p-4"
      >
    ...
    <div class="sm:w-full w-1/2 gap-2 flex flex-col">
      <div class="grid grid-cols-2 gap-2">
        <button
          @click="openSettlementFormModal"
          v-if="consignmentHistory && consignmentHistory.history.length > 0"
          class="col-span-1 hover:bg-white transition-colors duration-500 w-full h-[86px] grid place-items-center p-4 rounded-xl border-dashed border-2 border-border-dark"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            class="stroke-subtitle stroke-[1.5] w-6 h-6"
          >
            <path
      ...
      <ul v-if="consignmentHistory" class="w-full gap-2 flex flex-col-reverse justify-end">
        <template v-if="consignmentHistory.history.length > 0">
          <template v-for="(record, index) in consignmentHistory.history">
            <label
              v-if="
                ...
                new Date(record.transaction_on).getFullYear() !==
                  new Date(consignmentHistory.history[index - 1].transaction_on).getFullYear()
              "
            ...
            <li
              class="flex justify-between items-center p-4 bg-white rounded-xl border border-border-dark"
            >
              <div class="flex flex-col">
                <h3 class="font-medium">
                  ...
                  <span class="text-subtitle"
                    >&nbsp;{{ record.quantity }} {{ record.quantity > 1 ? 'copies' : 'copy' }}</span
                  >
                </h3>
                <h5 class="text-subtitle text-xs">
                  {{
                    record.type === 'settlement' || record.type === 'return'
                      ? record.receipt
                      : record.invoice
                  }}
                </h5>
                <label
                  ...
                  class="w-fit text-2xs leading-none font-semibold text-center whitespace-nowrap text-green-600"
                  >purchase</label
                >
                <label
                  v-if="record.type === 'sale'"
                  class="w-fit text-2xs leading-none font-semibold text-center whitespace-nowrap text-red-600"
                  >sale</label
                >
                <label
                  v-if="record.type === 'settlement'"
                  class="w-fit text-2xs leading-none font-semibold text-center whitespace-nowrap text-yellow-600"
                  >settlement</label
                >
                <label
                  v-if="record.type === 'return'"
                  class="w-fit text-2xs leading-none font-semibold text-center whitespace-nowrap text-gray-600"
                  >return</label
                >
              </div>
              <div class="flex flex-col items-center gap-1">
                <div
                  class="font-medium w-9 h-9 grid place-items-center bg-border-dark rounded-full"
                >
          ...
          <li class="px-2 py-2 font-medium text-subtitle flex justify-between">
            <span> Consignment History </span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              class="mx-2 stroke-2 stroke-subtitle w-4 h-4"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
        ...
        <li
          v-else
          class="min-h-24 flex flex-col items-center justify-center gap-2 font-medium text-subtitle py-8 h-fit"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
          ...
          <h5 class="font-normal sm:w-full w-[24rem] text-center">
            There has not been any consignment transactions made (purchase, sale or settlement) for
            this item. Consignment record history will appear here as they are made.
          </h5>
        </li>
      </ul>
    </div>
  ...
  <Teleport v-if="toggleNewSupplierForm" to="#top-view">
    <Modal>
      <div
        ref="newSupplierForm"
        class="w-[45%] sm:w-full sm:rounded-t-lg sm:overflow-auto sm:max-h-[75%] sm:animate-slide-up-modal animate-scale-up"
      >
        <Form
          modal
          class="h-full w-full rounded-b-none"
          :submit="submitNewSupplier"
          @cancel="addSupplier(false)"
          title-layout="contained"
          title="Assign a supplier"
          submit="assignSupplier"
        >
          <div class="w-full sm:flex-col flex gap-8">
            <TextInput
              required
              v-model="newSupplier.name"
              class="sm:w-full w-3/5"
              placeholder="Supplier full name"
              label="Full name"
            />
            <TextInput
              v-model="newSupplier.tin_number"
              class="sm:w-full w-2/5"
          ...
          <div class="w-full sm:flex-col flex gap-8">
            <TextInput
              ...
              class="sm:w-full w-1/2"
              placeholder="Phone number (09 / +251)"
              label="Phone number"
            />
            <TextInput
          ...
          <Collapsable
            ...
            :label="['Address information', 'Address information']"
          >
            <div class="w-full flex sm:flex-col gap-8">
              <TextInput
                v-model="newSupplier.address.sub_city"
                class="sm:w-full w-1/3"
                required
                placeholder="Sub-City"
                label="Sub-City"
              />
              <TextInput
                v-model="newSupplier.address.kebele"
                class="sm:w-full w-1/3"
                required
                placeholder="Kebele"
                label="Kebele"
              />
              <TextInput
                v-model="newSupplier.address.house_number"
                class="sm:w-full w-1/3"
            ...
            <div class="w-full sm:flex-col flex gap-8">
              <TextInput
                v-model="newSupplier.address.country"
                class="sm:w-full w-1/2"
                required
                placeholder="Country"
                label="Country"
  ...
  <ConsignmentSettlementForm
    @close="openSettlementFormModal(false)"
    :show="viewSettlementForm"
    v-if="consignmentHistory"
    :book="{
      title: consignmentHistory.book.title,
      id: consignmentHistory.book.id
    }"
  />

  <ConsignmentReturnForm
    @close="openReturnFormModal(false)"
    :show="viewReturnForm"
    v-if="consignmentHistory && viewReturnForm"
    :book="{
      title: consignmentHistory.book.title,
      id: consignmentHistory.book.id
    }"
  ...
  <InfoModalCard
    :open="viewInfoModalCard"
    @close="toggleInfoModalCardView"
    title="Why is the payable amount below zero?"
  >
    <template #description>
      Negative payable amount indicates the amount of items acquired from the consignee but paid in
      advance before sale of the items were made. Settlements to the consignee will only be eligible
      once the pre-purchases from are completed; i.e once the payable amount exceeds above a
      negative value.
    </template>
  </InfoModalCard>
</template>
...
function submitNewSupplier() {
  ...
  axios
    .post('/api/suppliers', { ...newSupplier.value })
    .then((response) => {
...
onMounted(() => {
  get({ book_id: useRoute().params.id }).then(() => {
    lastSettlement.value = consignmentHistory.value.history
      .slice()
      .reverse()
      .find((c) => {
