<template>
  <Modal>
    <Form
      ...
      subtitle="Upload the front and back covers of the book"
    >
        ...
        <div class="flex flex-col w-full gap-2">
          ...
          <template v-if="!!!covers['front_cover'] || !!replaceFrontCover">
            <FileInput
              ...
              class="!h-[28rem] md:!h-fit"
            />
          </template>
          <template v-else>
            <div
              class="group bg-wallpaper border-2 border-dashed grid place-items-center relative h-[28rem] md:h-fit border-border-light rounded-md p-4"
            >
              <button
                @click="replaceFrontCover = true"
                type="button"
                class="absolute grid place-items-center right-4 top-4 bg-brand-secondary opacity-0 group-hover:opacity-100 hover:bg-brand-primary rounded-full w-5 h-5"
              >
