<template>
  <div
    class="lg:w-full w-1/2 flex flex-col gap-6 bg-white rounded-xl shadow-sm bor min-h-[12rem] p-6"
  >
    ...
    >
      ...
      <DatePicker
        ...
        required
        v-model="formData.transferred_on"
      />
...
async function getStores() {
  return await axios
    .get('/api/stores')
    .then((response) => {
      stores.value = response.data.data.filter((store) => {
...
watch(
  ...
  () => {
    ...
    if (formData.value.from && formData.value.book_id) {
      getMaximumTransfer().then((response) => {
...
const formFilled = computed(() => {
  return !!formData.value.book_id && !!formData.value.quantity && !!formData.value.from
})
