<template>
  <div
    id="donutChart"
    class="chart-child snap-center min-w-[362.68px] flex flex-col bg-white rounded-md border border-border-light shadow-sm gap-4 min-h-[16rem] max-h-[256px]"
  >
    ...
    <div ref="canvasWrapper" class="relative grow w-full flex items-center justify-start sm:gap-0">
      <div class="ml-4 relative w-fit h-fit chart">
        <canvas
          class="top-0 left-0 absolute -rotate-90"
          width="125"
          height="125"
          ref="fullDonutCanvas"
        />
        <label
          class="w-[125px] h-[125px] text-border-light font-semibold text-2xl grid place-items-center"
        >
      ...
      <div
        ref="legendWrapper"
        class="grow relative scrollbar-chart snap-x snap-mandatory h-full pt-8 pb-4 px-4 flex overflow-auto gap-4"
      >
        <div
          :class="{
            'order-2': legends['first'].length < legends['second'].length
          }"
      ...
      <div
        class="absolute bottom-0 left-[125px] py-2 w-[calc(100%-125px)] flex items-center justify-center gap-2"
      >
        <div
          :class="[activeLegend === 'left' ? 'bg-subtitle' : 'bg-border-dark']"
          @click="scroll"
          class="hover:scale-125 cursor-pointer transition-transform duration-75 h-2 w-2 mt-1 rounded-full"
        />
        <div
          :class="[activeLegend === 'right' ? 'bg-subtitle' : 'bg-border-dark']"
          @click="scroll"
          class="hover:scale-125 cursor-pointer transition-transform duration-75 h-2 w-2 mt-1 rounded-full"
        />
      </div>
