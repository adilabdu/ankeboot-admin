<template>
  <div
    id="barChart"
    class="chart-child snap-center min-w-[362.68px] flex flex-col gap-4 bg-white rounded-md border border-border-light shadow-sm min-h-[16rem] max-h-[256px]"
  >
      ...
      <template v-for="i in 12">
        <div class="group flex flex-col gap-1 items-center justify-end text-center w-full">
          <div class="h-32 w-2 bg-border-dark rounded-full relative">
            <div
              v-if="values"
              :style="'height: ' + values[i - 1] + '%'"
              class="absolute w-full bottom-0 rounded-full bg-brand-primary z-10 animate-grow-up origin-bottom"
            ></div>
          </div>
          <label v-if="values === null" class="text-subtitle text-xs capitalize">N/A</label>
          <label
            v-else
            :class="[values[i - 1] === 100 ? 'text-brand-primary' : 'text-subtitle']"
