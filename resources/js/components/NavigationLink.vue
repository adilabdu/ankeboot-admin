<template>
  ...
  >
    ...
    <div :class="[mini ? 'hidden' : 'col-span-8']">
      <label :class="[isActive ? 'text-white font-medium' : 'text-black']" class="">{{
        label
      }}</label>
    </div>

    <div
      :class="[isActive ? 'bg-white' : 'bg-brand-secondary', mini ? 'hidden' : '']"
      v-if="number"
      class="absolute w-7 h-7 rounded-full right-2 flex items-center justify-center"
    >
