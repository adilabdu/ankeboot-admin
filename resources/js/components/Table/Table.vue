...
const displayData = computed(() => {
  ...
  if (
    page.value * limit.value >= filteredData.value.length &&
    limit.value !== filteredData.value.length
  ) {
    ...
    props.searchable.forEach((column) => {
      if (props.data[0][column] instanceof Date) {
        if (range === 'start') {
          found =
            found ||
            new Date(object[column].toLocaleDateString()) >=
              new Date(event.year, event.month, event.date)
          console.log({
            a: object[column].toLocaleDateString(),
            b: new Date(event.year, event.month, event.date).toLocaleDateString()
          })
        } else {
          found =
            found ||
...
<template>
  <div class="w-full flex flex-col sm:gap-4" :class="[title ? 'gap-4' : '']">
    ...
    <div class="flex flex-col gap-4">
      <!-- Filter Actions -->
      <div
        v-if="stringSearchable || dateSearchable || hideable"
      ...
      >
        ...
        <!-- Search -->
        <SearchBar
          v-if="stringSearchable"
          :placeholder="'Search for ' + arrayToStrings(searchable)"
          :min-query-length="0"
          @type="setStringQuery($event)"
        />
      ...
      <div class="rounded-md overflow-auto border-[0.5px] border-border-light">
        ...
        <table class="w-full bg-white shadow-sm">
          ...
          <tr
            class="hover:bg-black/5 h-10 border-border-light border-b-[1px]"
            v-if="!!filteredData && filteredData.length > 0"
            v-for="index in displayData"
            :key="index"
          >
            <slot
              name="rows"
        ...
        <div
          class="flex items-center justify-center min-h-[7.5rem] border-border-light border-b-[1px] w-full"
          v-if="!!filteredData && !!data && filteredData.length === 0 && data.length > 0"
        >
          <slot name="search-empty">
            <div class="gap-4 flex flex-col items-center justify-center py-8">
              <svg
      ...
      <div
        v-if="!!filteredData"
        class="w-full flex sm:flex-col items-center justify-between sm:gap-2"
      >
        <Dropdown
          :list="[5, 10, 25, 50, 'All']"
          v-model="showPerPage"
        ...
        <div class="flex gap-2 items-center justify-center sm:w-full">
          ...
          <button
            @click="paginateNext"
            :class="[
              page * limit < filteredData.length
                ? 'cursor-pointer'
      ...
      <!-- Summary -->
      <slot name="summary" :dateQuery="dateQuery" :data="filteredData"> </slot>
    </div>
  </div>
</template>
