<template>
  <div
    :class="[size === 'sm' ? 'min-h-[2.5rem]' : 'min-h-[3rem]']"
    class="border-b border-border-light last:border-none group items-center list-row grid grid-cols-12 relative"
  >
    <label
      :class="[size === 'sm' ? 'col-span-5' : label_size]"
      class="md:px-4 px-8 text-subtitle font-medium capitalize"
    >
      {{ label }}
    </label>
    ...
    <svg
      @click="copyText"
      :class="[clicked ? 'fill-white bg-subtitle opacity-100' : 'fill-subtitle bg-white']"
      class="copy-button transition-all duration-300 cursor-copy opacity-0 group-hover:opacity-100 transition-opacity duration-150 m-1 p-2 absolute right-0 rounded-md shadow-sm border-border-dark border-[0.5px] top-0"
      width="32"
      height="32"
      viewBox="0 0 20 20"
...
function copyText() {
  clicked.value = true
  navigator.clipboard
    .writeText(text.value.innerText)
    .then()
    .catch()
    .finally(() => {
      setTimeout(() => {
