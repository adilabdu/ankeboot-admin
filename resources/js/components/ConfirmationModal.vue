            ...
            <div class="w-full h-full flex justify-center items-center">
               ...
               <div class="animate-scale-up flex flex-col w-[32rem] overflow-hidden rounded-xl bg-white">
                    ...
                    <div class="p-6 flex items-center justify-end gap-2 h-16 w-full bg-wallpaper">
                        ...
                        <button @click="$emit('cancel')" class="focus:outline-none h-10 rounded-md font-medium bg-white text-black/75 border border-border-light px-5 focus:outline">
                            Cancel
                        </button>
                        <button :class="[loading ? 'opacity-50' : '']" @click="$emit('confirm')" class="flex items-center justify-center gap-2 focus:outline-none h-10 rounded-md bg-red-600 text-white px-6">
                            <svg v-if="loading" class="animate-rotate" width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
