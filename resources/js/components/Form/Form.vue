<template>
  <div
    :class="[titleLayout === 'panel' && largerThanSm ? 'grid grid-cols-12' : '']"
    class="w-full gap-2"
  >
    <div
      v-if="titleLayout !== 'contained' && largerThanSm"
      :class="[titleLayout === 'panel' && largerThanSm ? 'col-span-4' : '']"
    ...
    <form
      ...
      class="relative p-6 bg-white rounded-lg shadow-sm border border-border-light flex flex-col gap-6"
      :title="title"
      submit=""
    >
      <div
        v-if="isMounting"
        class="top-0 left-0 w-full h-full rounded-lg backdrop-blur absolute z-10"
      >
      ...
      >
        ...
        <div :class="[modal ? 'sm:w-full' : '']" class="flex gap-4">
          <button
            ...
            @click="$emit('cancel')"
            :class="[
              modal ? 'sm:w-full sm:bg-brand-secondary sm:border sm:border-brand-primary' : ''
            ]"
          ...
          <button
            :disabled="isLoading"
            type="submit"
            :class="[isLoading ? 'opacity-50' : '', modal ? 'sm:w-full' : '']"
            class="px-4 bg-brand-primary w-fit text-white rounded-md py-2"
          >
            <svg
              v-if="isLoading"
              class="inline animate-rotate mr-2"
              width="19"
              height="20"
              viewBox="0 0 19 20"
              fill="none"
...
const props = defineProps({
  ...
  subtitle: {
    type: String,
    default:
      'Fill in the information of the new book to be registered. Check book titles thoroughly to make sure it is not a duplicate entry.'
  },
  submit: {
    type: Function,
    required: true
