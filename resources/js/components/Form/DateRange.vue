<template>
  ...
  <div class="flex flex-col items-end gap-1 relative sm:w-full h-fit">
    ...
    <div
      ref="form"
      tabindex="0"
      @keyup.enter="togglePicker"
      @click="togglePicker"
      class="w-full focus:outline-2 focus:outline-brand-primary sm:focus:outline-none focus:outline-offset-2 border-[0.5px] border-border-light rounded-md sm:w-full bg-white h-10 shadow-sm flex items-center justify-between"
    >
      <p class="px-3" :class="[modelValue.date ? 'text-brand-primary' : 'text-subtitle']">
        {{ visibleDate }}
      </p>
      <input
        :value="visibleDate === 'dd MM YYYY' ? null : visibleDate"
        :required="required"
        class="sr-only"
      />
    ...
    <div
      :class="[align === 'left' ? 'left-0' : '']"
      ref="datepicker"
      v-if="showPicker"
      class="z-50 translate-y-full -mb-2 bottom-0 absolute min-w-[304.2px] overflow-hidden bg-white rounded-md shadow-md border-[0.5px] border-border-dark flex flex-col"
    >
      <div
        class="w-full flex justify-between items-center bg-[#F9FAFB] border-b border-border-dark shadow-sm"
      >
        <div
          @click="displayDatePicker ? previous() : () => {}"
          class="cursor-pointer p-4 flex items-center"
        >
          <button type="button">
            <svg
              :class="[!displayDatePicker ? 'opacity-25' : '']"
              class="fill-subtitle rotate-180"
              width="11"
              height="16"
              viewBox="0 0 11 16"
              xmlns="http://www.w3.org/2000/svg"
        ...
        <label class="grow text-center font-semibold text-subtitle">
          <span
            ...
            :class="[
              displayYearPicker ? 'hidden' : '',
              displayMonthPicker ? 'text-subtitle/50' : ''
            ]"
            @click="toggleWhichPicker('month')"
            >{{ months[month] }}</span
          >
          <span>&nbsp;</span>
          <span
            class="cursor-pointer"
            :class="[
              displayYearPicker ? 'text-subtitle/50' : '',
              displayMonthPicker ? 'text-subtitle/50' : ''
            ]"
            @click="toggleWhichPicker('year')"
            >{{ year }}</span
          >
        </label>
        <div
          @click="displayDatePicker ? next() : () => {}"
          class="cursor-pointer p-4 flex items-center"
        >
          <button type="button">
            <svg
      ...
      <button
        ...
        @click="setToToday"
        :class="[
          isSetToToday ? 'bg-brand-secondary border-brand-primary' : 'border-border-light',
          displayDatePicker ? '' : 'hidden'
        ]"
        class="flex justify-between items-center focus:outline-brand-primary sm:focus:outline-none focus:outline-offset-2 px-4 py-2 mx-3 rounded-md my-2 border-[1px] shadow-sm"
      >
        <span :class="[isSetToToday ? 'text-brand-primary' : 'text-subtitle']" class="font-medium"
          >Today&nbsp;</span
        >
        <span :class="isSetToToday ? 'text-brand-primary' : 'text-subtitle'" class="font-medium">{{
          today
      ...
      <div :class="[displayDatePicker ? '' : 'hidden']" class="grid grid-cols-7 grid-rows-7 p-2">
        <label
          v-for="i in days"
          class="font-medium text-subtitle p-2 border-2 border-transparent flex items-center justify-center rounded-md"
        >
          {{ i }}
        </label>

        <label
          v-for="_ in dateOffset"
          class="p-2 border-2 border-transparent flex items-center justify-center rounded-lg"
        >
          {{}}
        </label>
        ...
        <label
          tabindex="0"
          @keyup.enter="toggleSelectedDate(i, month, year)"
          @click="toggleSelectedDate(i, month, year)"
          v-for="i in numberOfDays"
          :class="[
            i === modelValue.date && month === modelValue.month && year === modelValue.year
              ? 'hover:bg-brand-primary font-medium text-white bg-brand-primary'
              : ''
          ]"
          class="font-medium cursor-pointer p-2 border-2 border-transparent flex items-center justify-center hover:bg-brand-secondary rounded-lg transition duration-150"
        >
          {{ i }}
        </label>
      </div>
      ...
      <div :class="[displayYearPicker ? '' : 'hidden']" class="grid grid-cols-4 grid-rows-7 p-2">
        <label
          tabindex="0"
          @keyup.enter="toggleSelectedYear(i)"
          @click="toggleSelectedYear(i)"
          v-for="i in availableYears"
          :class="[
            i === year ? 'hover:bg-brand-primary font-medium text-white bg-brand-primary' : ''
          ]"
          class="font-medium cursor-pointer p-2 border-2 border-transparent flex items-center justify-center hover:bg-brand-secondary rounded-lg transition duration-150"
        >
          {{ i }}
      ...
      <div :class="[displayMonthPicker ? '' : 'hidden']" class="grid grid-cols-3 p-2">
        <label
          tabindex="0"
          @keyup.enter="toggleSelectedMonth(index)"
          @click="toggleSelectedMonth(index)"
          v-for="(m, index) in months"
          :class="[
            index === month ? 'hover:bg-brand-primary font-medium text-white bg-brand-primary' : ''
          ]"
          class="font-medium cursor-pointer p-2 border-2 border-transparent flex items-center justify-center hover:bg-brand-secondary rounded-lg transition duration-150"
        >
          {{ m }}
...
const isSetToToday = computed(() => {
  ...
  return (
    today.getFullYear() === props.modelValue.year &&
