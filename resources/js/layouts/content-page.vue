<template>
  <div
    id="contentPage"
    class="scrollbar-brand overflow-y-auto grow px-12 py-8 sm:px-4 xs:px-2 gap-8 flex flex-col w-full"
  >
    <div
      class="print:hidden px-4 gap-3 -mb-3 bg-white w-fit min-h-[2rem] flex items-center justify-center border border-border-light shadow-sm rounded-lg"
    >
      <template v-for="(route, index) in routes">
        <RouterLink
          class="hover:text-brand-primary focus:outline-none"
          v-if="route === '' && index !== routes.length - 1"
          :to="{ name: 'Dashboard' }"
          >Home</RouterLink
        >
        <RouterLink
          :class="{
            'text-brand-primary': index === routes.length - 1
          }"
          :to="{ path: getSubRoute(index) }"
          v-else
          class="capitalize hover:text-brand-primary focus:outline-none"
          >{{
            route === ''
  ...
  <Teleport v-if="contentLoading" to="#content-top-view">
    <Modal
      :class="[authenticated ? 'w-[calc(100%-16rem)]' : 'w-full']"
      class="xl:w-full !bg-wallpaper"
    >
      <svg
...
function getSubRoute(i) {
  ...
  routes.value.forEach((route, index) => {
    if (index < i) {
      subRoute += `${route}/`
    } else if (index === i) subRoute += route
  })
