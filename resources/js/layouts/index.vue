<template>
  <div
    :class="progress.percentage === 0 ? 'opacity-0 duration-700 transition-opacity' : ''"
    :style="{ width: 100 - progress.percentage + '%' }"
    class="transition-all duration-300 fixed top-0 left-0 h-[0.25rem] bg-brand-primary z-50"
  />
  ...
  <div class="flex flex-row max-w-full relative overflow-x-hidden">
      ...
      <div class="flex flex-col justify-between h-full">
        <div class="flex flex-col">
          <NavigationLink :minimize="miniNav" v-slot="props" label="dashboard" route="">
            <PresentationChartLineIcon
              class="w-6 h-6"
              :class="[props.active ? 'stroke-white' : 'stroke-black', 'stroke-[1.5] scale-90']"
            />
          </NavigationLink>
          <NavigationLink :minimize="miniNav" v-slot="props" label="books">
            <BookOpenIcon
              class="w-6 h-6 scale-90 stroke-[1.5]"
              :class="[props.active ? 'stroke-white' : 'stroke-black']"
            />
          </NavigationLink>
          ...
          <NavigationLink :minimize="miniNav" v-slot="props" label="stores">
            <BuildingStorefrontIcon
              class="w-6 h-6 stroke-[1.5] scale-90"
              :class="[props.active ? 'stroke-white' : 'stroke-black']"
            />
          </NavigationLink>
          <NavigationLink
            :minimize="miniNav"
            :number="store.state.daily_sale.unsubmitted"
            v-slot="props"
            label="daily sales"
            route="daily-sales"
          >
            <CalendarDaysIcon
              class="w-6 h-6 scale-90"
              :class="[props.active ? 'stroke-white stroke-2' : 'stroke-black stroke-[1.5]']"
            />
          </NavigationLink>
          <NavigationLink
            :minimize="miniNav"
        ...
        <div class="flex flex-col items-center pt-8 gap-2">
          ...
          <h1 class="flex gap-1 text-xs text-subtitle text-center">
            <span> &copy; </span>
            <span :class="{ hidden: miniNav }">
              {{ footerWords()[getRandomInt(footerWords().length)] }}.
            </span>
            <span> 2023 </span>
          </h1>
        </div>
    ...
    <div
      :class="[
        authenticated ? (miniNav ? 'w-[calc(100%-63px)]' : 'w-[calc(100%-16rem)]') : 'w-full'
      ]"
      class="min-h-full xl:w-full flex flex-col"
    >
      <Header />

      <div class="grow overflow-y-auto flex flex-col relative overflow-overlay">
        <div
          class="h-fit max-h-full min-w-96 m-2 absolute right-0 flex flex-col items-end gap-2 z-50"
        >
...
const remote = ref(localStorage.getItem('connection') === 'remote_mysql')
...
watch(remote, () => {
  ...
  remote.value
    ? localStorage.setItem('connection', 'remote_mysql')
